...
def get_sampling_infinite_horizon_controller(dx, vf, ctrl_model):
    # num_samples = 50
    num_samples = 10
    u_dim = 1
    dtype = torch.float64
    ...
    def ctrl(x):
        ...
        for k in range(num_samples):
            # u = np.random.rand(u_dim) * 20. - 10.
            # u = np.random.rand(u_dim) * 2000. - 1000.
            u = np.random.rand(u_dim) * 200. - 100.
            sim_dyn = lambda t, y: dx(y, u)
            traj = scipy.integrate.solve_ivp(sim_dyn, (0, dt), x0_np)
            if traj.success:
                step_cost = vf.step_cost(0, x, torch.Tensor(u).double())
                xn = torch.Tensor(traj.y[:,-1]).double()
                v = step_cost + torch.clamp(ctrl_model(xn), 0.)
                if v < v_opt:
                    v_opt = v
                    u_opt = u
        u_opt = torch.Tensor(u_opt).double()
