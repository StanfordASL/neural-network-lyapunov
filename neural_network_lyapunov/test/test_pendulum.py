...
class TestPendulum(unittest.TestCase):
    def test_dynamics_gradient(self):
        ...
        def test_gradient(x, u):
            ...
            (A_expected0, B_expected0) = torch.autograd.grad(
                xdot, (x, u), grad_outputs=torch.tensor([1, 0], dtype=dtype),
                retain_graph=True)
            (A_expected1, B_expected1) = torch.autograd.grad(
                xdot, (x, u), grad_outputs=torch.tensor([0, 1], dtype=dtype),
                retain_graph=True)
            A_expected = torch.cat(
                (A_expected0.reshape((1, -1)), A_expected1.reshape((1, -1))),
                dim=0)
            B_expected = torch.cat(
                (B_expected0.reshape((1, -1)), B_expected1.reshape((1, -1))),
                dim=0)
            A, B = dut.dynamics_gradient(x)
            np.testing.assert_allclose(
                A_expected.detach().numpy(), A.detach().numpy())
            np.testing.assert_allclose(
                B_expected.detach().numpy(), B.detach().numpy())

        test_gradient(
            torch.tensor([0, 1], dtype=dtype, requires_grad=True),
            torch.tensor([2], dtype=dtype, requires_grad=True))
        test_gradient(
            torch.tensor([3, 1], dtype=dtype, requires_grad=True),
            torch.tensor([2], dtype=dtype, requires_grad=True))
        test_gradient(
            torch.tensor([3, -1], dtype=dtype, requires_grad=True),
            torch.tensor([2], dtype=dtype, requires_grad=True))
    ...
    def test_swing_up_control(self):
        ...
        for x0 in x0s:
            result = scipy.integrate.solve_ivp(
                lambda t, x: plant.dynamics(x, controller(x)), (0, 10), x0,
                events=converged)
