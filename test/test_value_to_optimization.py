...
class ValueToOptimizationTest(unittest.TestCase):
    def test_trajopt_x0xN(self):
        ...
        [Q, R, Z, q, r, z, Qt, Rt, Zt, qt, rt, zt] = get_simple_trajopt_cost(sys.x_dim, sys.u_dim, sys.num_modes, dtype)
        ...
        traj = np.reshape(traj, ((1 + sys.num_modes)*(sys.x_dim + sys.u_dim), N), order='F')
    ...
    def test_trajopt_obj(self):
        ...
        slack0 = torch.rand(sys.num_modes*(sys.x_dim + sys.u_dim)).type(sys.dtype)
        slack = torch.rand(sys.num_modes*(sys.x_dim + sys.u_dim), N-1).type(sys.dtype)
        s = torch.cat((u0, slack0, torch.cat((x, u, slack), 0).t().reshape(-1)))
        ...
        obj = .5*s@Q2@s.t() + .5*alpha_flat@Q3@alpha_flat.t() + s@q2 + alpha_flat@q3 + c
