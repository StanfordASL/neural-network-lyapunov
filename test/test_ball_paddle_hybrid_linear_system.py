...
class BallPaddleHybridLinearSystemTest(unittest.TestCase):
    def setUp(self):
        ...
        self.dt = .0025
        self.x_lo = torch.Tensor([-1.,0.,0.,-np.pi/2,-1000.,-1000.,-1000.]).type(self.dtype)
        self.x_up = torch.Tensor([1000.,2000.,1.,np.pi/2,10000.,10000.,10000.]).type(self.dtype)
        self.u_lo = torch.Tensor([-1000.,-1000.]).type(self.dtype)
        self.u_up = torch.Tensor([1000.,1000.]).type(self.dtype)
        self.sys = ball_paddle.get_ball_paddle_hybrid_linear_system(self.dtype, self.dt, self.x_lo, self.x_up, self.u_lo, self.u_up)

    def test_ball_paddle_sim(self):
        (Aeq_slack, Aeq_alpha, 
         Ain_x, Ain_u, Ain_slack, Ain_alpha, 
         rhs_in) = torch_to_numpy(self.sys.mixed_integer_constraints(self.x_lo, self.x_up, self.u_lo, self.u_up))
