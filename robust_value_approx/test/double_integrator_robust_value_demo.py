    ...
    for epoch in range(num_epoch):
        for batch_data, batch_label in train_data_loader:
            ...
            if n_iter >= pre_training_iter:
                ...
                epsilon = mb_prob.compute_objective_from_mip_data_and_solution(
                    penalty=1e-8)
            else:
            ...
            if n_iter >= pre_training_iter:
                ...
                mb_prob.gurobi_model.optimize()
                robust_epsilon =\
                    mb_prob.compute_objective_from_mip_data_and_solution(
                        penalty=1e-8)
            else:
            ...
            print("===")
            n_iter += 1
    epsilon_log[:pre_training_iter] = [None] * (pre_training_iter)
    robust_epsilon_log[:pre_training_iter] = [None] * (pre_training_iter)
    plt.plot(l2_log)
