...
def setup_state_samples_all(x_equilibrium, lower, upper, mesh_size, theta):
    ...
    (samples_x, samples_y) = torch.meshgrid(
        torch.linspace(lower[0] - x_equilibrium[0],
                       upper[0] - x_equilibrium[0],
                       mesh_size[0],
                       dtype=dtype),
        torch.linspace(lower[1] - x_equilibrium[1],
                       upper[1] - x_equilibrium[1],
                       mesh_size[1],
                       dtype=dtype))
...
def setup_state_samples_on_boundary(x_equilibrium, lower, upper, mesh_size,
                                    theta):
    """
        ...
        for i in range(theta.shape[0] - 1):
            ...
            for j in range(P.shape[0]):
                ...
                if torch.all(P @ ray <= 1e-12):
                    ax.plot([0, ray[0] / ray_magnitude * 10],
...
def plot_lyapunov(ax, relu, V_lambda, x_equilibrium, R, x_lower, x_upper,
                  mesh_size, fontsize, **kwargs):
    """
    Plot V(x) in 3D.
    where V(x) = relu(x) + ρ|x-x*|₁
    """
    with torch.no_grad():
        state_samples_all = setup_state_samples_all(x_equilibrium, x_lower,
                                                    x_upper, mesh_size, 0.)
...
def plot_lyapunov_colormap(fig, ax, relu, V_lambda,
                           lyapunov_positivity_epsilon, x_equilibrium, R,
                           x_lower, x_upper, mesh_size, title_fontsize,
                           **kwargs):
    """
    ...
    with torch.no_grad():
        ...
        samples_x_np = samples_x.detach().numpy()
        samples_y_np = samples_y.detach().numpy()
        plot = ax.pcolor(samples_x_np, samples_y_np, V_minus_l1_np, **kwargs)
        rc('text', usetex=True)
...
def plot_lyapunov_dot_colormap(fig, ax, relu, system, V_lambda,
    ...
    with torch.no_grad():
        ...
        for i in range(mesh_size[0]):
            for j in range(mesh_size[1]):
                state_sample = torch.tensor([samples_x[i, j], samples_y[i, j]],
                                            dtype=dtype)
                dV[i, j] = torch.max(
                    torch.cat(
        ...
        plot = ax.pcolor(samples_x_np, samples_y_np, dV_np, **kwargs)
        ax.set_xlabel("x(1)", fontsize=fontsize)
        ax.set_ylabel("x(2)", fontsize=fontsize)
        if discrete_time:
...
def plot_sublevel_set(ax, relu, V_lambda, x_equilibrium, R, upper_bound,
                      **kwargs):
    """
...
def plot_phase_portrait(ax, system, x_lower, x_upper, mesh_size, fontsize,
                        **kwargs):
    """
...
def plot_cost_to_go_approximator(relu, x0_value_samples, x_equilibrium,
                                 V_lambda, R, lower, upper, mesh_size, theta):
    """
    ...
    dtype = torch.float64
    with torch.no_grad():
        state_samples_all = setup_state_samples_all(x_equilibrium, lower,
                                                    upper, mesh_size, theta)
        samples_x = torch.empty(mesh_size)
        samples_y = torch.empty(mesh_size)
        for i in range(mesh_size[0]):
            for j in range(mesh_size[1]):
        ...
        R = torch.tensor([[c_theta, -s_theta], [s_theta, c_theta]],
                         dtype=dtype)
        ...
        x0_value_samples_np = np.array([[pair[0][0], pair[0][1], pair[1]]
                                        for pair in x0_value_samples
                                        if is_point_in_box(pair[0])])
        ax1.plot(x0_value_samples_np[:, 0],
                 x0_value_samples_np[:, 1],
                 x0_value_samples_np[:, 2],
                 linestyle='None',
                 marker='.',
                 markersize=1.)
