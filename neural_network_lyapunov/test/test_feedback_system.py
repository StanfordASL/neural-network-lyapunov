...
class TestFeedbackSystem(unittest.TestCase):
    def setUp(self):
        ...
        self.controller_network1 = utils.setup_relu(
            (3, 3, 3, 2), controller_network_params1, negative_slope=0.01,
    ...
    def add_dynamics_mip_constraint_tester(
        ...
        dut = feedback_system.FeedbackSystem(
            forward_system, controller_network, x_equilibrium,
        ...
        x_next = milp.addVars(
            dut.x_dim, lb=-gurobipy.GRB.INFINITY,
            vtype=gurobipy.GRB.CONTINUOUS, name="x_next")
    ...
    def step_forward_at_equilibrium_test(
        ...
        with torch.no_grad():
            ...
            np.testing.assert_allclose(
                x_next.detach().numpy(), x_equilibrium.detach().numpy())

    def step_forward_test(
        self, forward_system, controller_network, x, x_equilibrium,
            u_equilibrium, u_lower_limit, u_upper_limit):
        closed_loop_system = feedback_system.FeedbackSystem(
            forward_system, controller_network, x_equilibrium,
            u_equilibrium, u_lower_limit, u_upper_limit)
    ...
    def test_step_forward_hybrid_linear_system(self):
        ...
        self.step_forward_test(
            ...
            u_equilibrium, np.array([-np.inf, -np.inf]),
    ...
    def test_step_forward_relu_second_order_system_given_equilibrium(self):
        ...
        self.step_forward_at_equilibrium_test(
            forward_system, self.controller_network2,
            forward_system.x_equilibrium,
            u_equilibrium, np.array([-np.inf]), np.array([np.inf]))
        self.step_forward_at_equilibrium_test(
            forward_system, self.controller_network2,
            forward_system.x_equilibrium,
