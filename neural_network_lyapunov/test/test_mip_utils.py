...
class TestComputeRangeByLP(unittest.TestCase):
    def test_x_bounds(self):
        ...
        b = np.array([0., 1., 3.])
        y_lb, y_ub = mip_utils.compute_range_by_lp(
            A, b, x_lb, x_ub, None, None)
...
class TestComputeRangeByIA(unittest.TestCase):
    def test(self):
        ...
        np.testing.assert_allclose(
            output_lb.detach().numpy(), np.array([-15, -34]))
        np.testing.assert_allclose(
            output_ub.detach().numpy(), np.array([25, 18]))
    ...
    def test_gradient(self):
        ...
        dtype = torch.float64
        A = torch.tensor(
            [[1., 2., -3.], [2., -1., -4.]], dtype=dtype, requires_grad=True)
        ...
        np.testing.assert_allclose(
            A_grad, A_grad_numerical.reshape((2, 3)), atol=1E-6)
...
class TestPropagateBounds(unittest.TestCase):
    ...
    def test_linear_layer_no_bias_LP(self):
        ...
        output_lo_expected, output_up_expected = mip_utils.compute_range_by_lp(
            ...
            input_lo.detach().numpy(), input_up.detach().numpy(), None, None)
        np.testing.assert_allclose(
            output_lo.detach().numpy(), output_lo_expected)
        np.testing.assert_allclose(
            output_up.detach().numpy(), output_up_expected)
