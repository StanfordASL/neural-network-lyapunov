...
class TestControlLyapunovFixedActivationPath(unittest.TestCase):
    def test_constructor(self):
        ...
        dut = ControlLyapunov.ControlLyapunovFixedActivationPath(g, P, q, A, B, u_vertices)
        prob = dut.construct_program()
        prob.solve()
        self.assertEqual(prob.status, cp.OPTIMAL)
        self.assertTrue(np.less_equal(P.dot(dut.x.value), q).all())
        self.assertAlmostEqual(prob.value, g.T.dot(dut.x.value) + np.min(g.T.dot(B.dot(u_vertices))), 5)
