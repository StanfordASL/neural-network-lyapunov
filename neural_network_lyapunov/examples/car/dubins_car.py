...
class DubinsCarReLUModel:
    ...
    [delta_pos_x, delta_pos_y] = ϕ(θ, vel) - ϕ(0, 0)
    where vel is the velocity along the car heading direction.
    """
    # TODO(hongkai.dai): Strictly speaking, delta_pos_x, delta_pos_y should
    # also depend on the car angular rate. Here we dropped this dependency to
    # get a simpler neural network. If this neural network doesn't give us a
    # good controller, we will add the angular rate also as an input to the
    # neural network.
    def __init__(
    ...
    def step_forward(self, x_start: torch.Tensor, u_start: torch.Tensor) ->\
        ...
        if len(x_start.shape) == 1:
            ...
            delta_position = self.dynamics_relu(torch.stack(
                (x_start[2], u_start[0]))) - self.dynamics_relu(
            ...
            # batch of data
            delta_position = self.dynamics_relu(torch.cat(
                (x_start[:, 2].reshape((-1, 1)),
                 u_start[:, 0].reshape((-1, 1))), dim=1)) - self.dynamics_relu(
                     torch.zeros((2,), dtype=self.dtype))
            position_next = x_start[:, :2] + delta_position
    ...
    def add_dynamics_constraint(
        self, mip: gurobi_torch_mip.GurobiTorchMIP, x_var, x_next_var, u_var,
        ...
        mip_cnstr_result, _, _, _, _ = \
            self.dynamics_relu_free_pattern.output_constraint(
                torch.stack((self.x_lo[2], self.u_lo[0])),
                torch.stack((self.x_up[2], self.u_up[0])))
        ...
        # [delta_pos_x, delta_pos_y] = ϕ(θ[n], vel[n])−ϕ(0, 0)
        ...
        mip.addMConstrs(
            ...
            b=mip_cnstr_result.Cout - self.dynamics_relu(torch.zeros(
                (2,), dtype=self.dtype)), sense=gurobipy.GRB.EQUAL,
            name="dubins_car_forward_dynamics_output")

        # Now add the constraint θ[n+1] = θ[n] + θ_dot[n] * dt
        mip.addLConstr(
            [torch.tensor([1., -1., -self.dt], dtype=self.dtype)],
            [[x_next_var[2], x_var[2], u_var[1]]], sense=gurobipy.GRB.EQUAL,
            rhs=0., name="dubins_car_theta_dynamics")
