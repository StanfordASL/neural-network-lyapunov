...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_forward_model",
                        type=str,
                        default=None,
    ...
    V_lambda = 0.9
    x_lo = torch.tensor(
        [-.25, -.25, -.25, -np.pi / 4, -np.pi / 4, -np.pi / 4, -1, -1, -1],
        dtype=dtype)
    x_up = torch.tensor(
        [.25, .25, .25, np.pi / 4, np.pi / 4, np.pi / 4, 1, 1, 1], dtype=dtype)
    ...
    if args.load_folder:
        ...
        R = torch.cat((.1 * torch.eye(9, dtype=dtype), .123 * torch.ones(
            (6, 9), dtype=dtype)),
                      dim=0)
        lyapunov_relu = utils.setup_relu((9, 12, 6, 1),
                                         params=None,
                                         negative_slope=0.1,
                                         bias=True,
                                         dtype=dtype)
        controller_relu = utils.setup_relu((9, 7, 4),
                                           params=None,
                                           negative_slope=0.01,
                                           bias=True,
                                           dtype=dtype)
