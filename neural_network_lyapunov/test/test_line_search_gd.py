    ...
    def test(self):
        ...
        def grad_fun(a, b):
            with torch.no_grad():
                ...
                return torch.cat([(2 * a.reshape(
                    (1, -1)) @ X).squeeze() + 2 * b * x_sum,
        ...
        np.testing.assert_allclose(
            ...
            grad_fun(network.weight.data.squeeze(),
                     network.bias).detach().numpy())
        # Take a single step
        lr = 1e-3
        ...
        assert (loss1 <= loss0 - loss_minimal_decrement * lr * grad0 @ grad0)
        ...
        np.testing.assert_almost_equal(loss2.item(), loss_fun(a, b).item())
        ...
        np.testing.assert_allclose(
            ...
            a.detach().numpy())
        np.testing.assert_almost_equal(network.bias.data.item(), b.item())
        np.testing.assert_almost_equal(loss3.item(), loss_fun(a, b).item())
        self.assertGreater(
            loss3.item(),
            loss0.item() + (loss_minimal_decrement * step_size /
                            step_size_reduction * -grad0 @ grad0).item())
        ...
        np.testing.assert_allclose(network(x).squeeze().detach().numpy(),
                                   np.zeros(3),
                                   atol=1e-3)
