...
class BallPaddleSystemSim(unittest.TestCase):
    def test_transcription(self):
        ...
        sys = BallPaddleSystem.BallPaddleSystem()
        ...
        def eval_eq(i):
            return (Aeq1[i,:]@x + Aeq2[i,:]*u + Aeq3[i,:]@xp + Aeq4[i,:]*up + Aeq5[i,:]*a - rhs_eq[i])[0]
        def eval_in(i):
            return (Ain1[i,:]@x + Ain2[i,:]*u + Ain3[i,:]@xp + Ain4[i,:]*up + Ain5[i,:]*a - rhs_in[i])[0]
    
        self.assertAlmostEqual(zp + u*dt*.5 - zpp + up*dt*.5, eval_eq(0), places=6)
        self.assertAlmostEqual(zb + zbdot*dt*.5 - zbp + zbdotp*dt*.5, eval_eq(1), places=6)
        self.assertAlmostEqual(zbdot - zbdotp - M*a + g*dt, eval_in(0), places=6) 
        self.assertAlmostEqual(- zbdot + zbdotp - M*a - g*dt, eval_in(1), places=6)
        self.assertAlmostEqual(-cr*zbdot + (cr+1.)*u - zbdotp + M*a - M, eval_in(2), places=6)
        self.assertAlmostEqual(cr*zbdot -(1+cr)*u + zbdotp + M*a - M, eval_in(3), places=6)
        self.assertAlmostEqual(-zp + zb + dt*zbdot - u*dt*.5 - up*dt*.5 + M*a - M + .5*g*dt*dt, eval_in(4), places=5)
    ...
    def test_simulation(self):
        ...
        for i in range(N-1):
            con += [
                Aeq1@x[:,i] + Aeq2@np.array([u]) + Aeq3@x[:,i+1] + Aeq4@np.array([u]) + Aeq5@a[:,i] == rhs_eq[:,0],
                Ain1@x[:,i] + Ain2@np.array([u]) + Ain3@x[:,i+1] + Ain4@np.array([u]) + Ain5@a[:,i] <= rhs_in[:,0],
            ]
        obj = cp.Minimize(0.)
        ...
        self.assertFalse(type(x.value) == type(None))
